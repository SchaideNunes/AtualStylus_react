...
import { X, Plus, User, Phone, Calendar, Clock, Scissors, Check, AlertCircle } from 'lucide-react';
import { api } from '../services/api';
import { getDataHojeString, isDomingo } from '../utils/dateUtils';
import { formatarTelefone } from '../utils/phoneMask';

export function ModalNovoAgendamento({ isOpen, onClose, onSalvar }) {
  ...
  // Carregar slots disponíveis dinamicamente
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!data) {
      setErro('Por favor, selecione a data do atendimento.');
      return;
    }
    if (!horario) {
  ...
  const modalJSX = (
    <div className="modal-overlay" onClick={onClose}>
      <div 
        ...
        style={{ borderRadius: '24px', background: '#141414', border: '1px solid #2e2e2e', maxWidth: '620px' }}
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px', borderBottom: '1px solid #242424', paddingBottom: '14px' }}>
          <h3 style={{ color: '#ffffff', fontSize: '1.25rem', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '8px', margin: 0, textTransform: 'uppercase' }}>
            <Plus color="#ffffff" size={22} strokeWidth={3} /> Novo Agendamento
          </h3>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
              <div className="campo-caixa-limpo">
                <span className="rotulo-campo-limpo">
                  {carregandoHorarios ? 'CARREGANDO HORÁRIOS...' : 'HORÁRIO DISPONÍVEL'}
                </span>
                <div className="linha-input-limpo">
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', borderTop: '1px solid #242424', paddingTop: '16px', marginTop: '6px' }}>
            ...
            >
              <Check size={18} /> {salvando ? 'Cadastrando...' : 'Confirmar Agendamento'}
            </button>
          </div>
        </form>
      </div>
